...
@Component(service = WorkflowProcess.class, property = { "process.label=Replicate to Preview Process" })
@Designate(ocd = PreviewProcess.PreviewProcessConfig.class)
public class PreviewProcess implements WorkflowProcess {

	private static final Logger log = LoggerFactory.getLogger(PreviewProcess.class);
	...
	@ObjectClassDefinition(name = "Preview Configuration", description = "Preview Configure Review ie. preview agent name")
	public @interface PreviewProcessConfig {

		@AttributeDefinition(name = "Preview replication agent name", description = "Configure Preview agent name")
		String getPreviewReplicationAgentName() default "preview";
	}

	@Activate

	public void active(PreviewProcessConfig previewProcessConfig) {

		log.debug("*********Active Method**************");
		ReplicateAgentName = previewProcessConfig.getPreviewReplicationAgentName();
	...
	private void replicateContent(Session session, String contentPath) throws LoginException {
		...
		try {
			...
			log.error("Error while replicating the content", e);
		}
